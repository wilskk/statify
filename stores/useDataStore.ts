        ...
        immer((set, get) => ({
            ...
            ensureMatrixDimensions: (maxRow, maxCol, minColCount) => {
                set((state) => {
                    ...
                    if (maxRow >= state.data.length) {
                        const colsCount = Math.max(state.data[0].length, effectiveMaxCol + 1);
            ...
            updateCell: async (row, col, value) => {
                ...
                if (row < currentData.length && col < currentData[0]?.length) {
                    if (currentData[row][col] === value) return;
                }
                set((state) => {
            ...
            loadData: async () => {
                ...
                try {
                    ...
                    await db.transaction('r', db.cells, async () => {
                        ...
                        cells.forEach((cell) => { availableMatrix[cell.row][cell.col] = cell.value; });
                    });
                    set((state) => { state.data = availableMatrix; state.lastUpdated = new Date(); state.isLoading = false; });
                } catch (error: any) {
            ...
            addRow: async (index?) => {
                set((state) => {
                    ...
                    state.data = [...state.data.slice(0, rowIndex), newRow, ...state.data.slice(rowIndex)];
                    state.lastUpdated = new Date();
                });
            ...
            addColumn: async (index?) => {
                set((state) => {
                    const colIndex = index !== undefined ? index : (state.data.length > 0 ? state.data[0].length : 0);
            ...
            deleteRow: async (index: number) => {
                set((state) => {
                    if (index < 0 || index >= state.data.length) { state.error = { message: "Invalid row index", source: "deleteRow" }; return; }
            ...
            deleteColumn: async (index: number) => {
                set((state) => {
                    if (state.data.length === 0 || index < 0 || (state.data.length > 0 && index >= state.data[0].length)) { state.error = { message: "Invalid column index", source: "deleteColumn" }; return; }
                    state.data = state.data.map(row => {
                        const newRow = [...row];
            ...
            validateVariableData: async (columnIndex: number, type: string, width: number) => {
                ...
                const result = { isValid: true, issues: [] as Array<{ row: number; message: string }> };
                ...
                for (let row = 0; row < data.length; row++) {
                    ...
                    if (type.startsWith("NUMERIC") || ["COMMA", "DOT", "SCIENTIFIC"].includes(type)) {
                        if (typeof value !== 'number' && isNaN(Number(value))) {
                            result.issues.push({ row, message: `Row ${row + 1}: Value "${value}" is not a valid number` });
                            result.isValid = false; updates.push({ row, col: columnIndex, value: "" }); continue;
                        }
                        const numValue = typeof value === 'number' ? value : Number(value);
                        const valueStr = numValue.toString();
                        if (valueStr.length > width) {
                            result.issues.push({ row, message: `Row ${row + 1}: Number "${value}" exceeds width (${valueStr.length} digits, max allowed is ${width})` });
                            result.isValid = false; updates.push({ row, col: columnIndex, value: "" });
                        }
                    } else if (type === "STRING") {
                        const strValue = String(value);
                        if (strValue.length > width) {
                            result.issues.push({ row, message: `Row ${row + 1}: String "${strValue}" is too long (${strValue.length} chars, max allowed is ${width})` });
                            result.isValid = false; updates.push({ row, col: columnIndex, value: strValue.substring(0, width) });
                        }
                    } else if (["DATE", "ADATE", "EDATE", "SDATE", "JDATE"].includes(type)) {
                        const dateStr = String(value);
                        const isValidDate = !isNaN(Date.parse(dateStr));
                        if (!isValidDate) {
                            result.issues.push({ row, message: `Row ${row + 1}: Value "${dateStr}" is not a valid date` });
                            result.isValid = false; updates.push({ row, col: columnIndex, value: "" });
                        }
                    }
                }
            ...
            resetData: async () => {
                try {
                    await db.transaction('rw', db.cells, async () => { await db.cells.clear(); });
                    set((state) => { state.data = []; state.lastUpdated = new Date(); });
                } catch (error: any) {
            ...
            getVariableData: async (variable: Variable) => {
                if (get().data.length === 0) { await get().loadData(); }
                const { columnIndex } = variable;
                const storedData = get().data;
                const columnData = storedData.map(row => columnIndex < row.length ? row[columnIndex] : "");
            ...
            updateBulkCells: async (updates) => {
                ...
                const filteredUpdates = updates.filter(({ row, col, value }) => {
                    if (row < data.length && col < data[row].length) { return data[row][col] !== value; }
                    return true;
                });
                if (filteredUpdates.length === 0) return;
                let maxRow = 0; let maxCol = 0;
                for (const { row, col } of filteredUpdates) { maxRow = Math.max(maxRow, row); maxCol = Math.max(maxCol, col); }
                set((state) => {
                    get().ensureMatrixDimensions(maxRow, maxCol);
                    for (const { row, col, value } of filteredUpdates) { state.data[row][col] = value; }
                    state.lastUpdated = new Date();
                });
                try {
                    await db.transaction('rw', db.cells, async () => {
                        ...
                        if (emptyUpdates.length > 0) {
                            await Promise.all(emptyUpdates.map(({ row, col }) => db.cells.where('[col+row]').equals([col, row]).delete()));
                        }
                        if (nonEmptyUpdates.length > 0) { await db.cells.bulkPut(nonEmptyUpdates); }
                    });
                } catch (error: any) {
                    console.error("Bulk update failed:", error);
                    set((state) => { state.error = { message: "Bulk update failed", source: "updateBulkCells", originalError: error }; });
                    await get().loadData();
                }
            },
            ...
            setDataAndSync: async (newData) => {
                ...
                set((state) => { state.data = newData; state.lastUpdated = new Date(); });
                try {
                    await db.transaction('rw', db.cells, async () => {
                        ...
                        for (let row = 0; row < newData.length; row++) {
                            for (let col = 0; col < newData[row].length; col++) {
                                if (newData[row][col] !== "") { cells.push({ row, col, value: newData[row][col] }); }
                            }
                        }
                        if (cells.length > 0) { await db.cells.bulkPut(cells); }
                    });
                } catch (error: any) {
                    console.error("Failed to sync data:", error);
                    set((state) => { state.error = { message: "Failed to sync data with database", source: "setDataAndSync", originalError: error }; });
                    await get().loadData();
                }
            },
            ...
            sortData: async (columnIndex: number, direction: 'asc' | 'desc') => {
                ...
                set(state => { state.data = rowsToSort; state.lastUpdated = new Date(); });
            ...
            swapRows: async (row1: number, row2: number) => {
                set((state) => {
                    if (row1 < 0 || row2 < 0 || row1 >= state.data.length || row2 >= state.data.length) {
                        state.error = { message: "Invalid row indices for swapping", source: "swapRows" }; return;
                    }
            ...
            swapColumns: async (col1: number, col2: number) => {
                set((state) => {
                    ...
                    if (col1 < 0 || col2 < 0 || col1 >= state.data[0].length || col2 >= state.data[0].length) {
                        state.error = { message: "Invalid column indices for swapping", source: "swapColumns" }; return;
                    }
                    for (let i = 0; i < state.data.length; i++) {
