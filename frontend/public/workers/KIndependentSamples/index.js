...
import { resultKruskalWallisH } from './testStatisticsKruskalWallis.js';
...
self.onmessage = (e) => {
    ...
    try {
        ...
        if (testType.kruskalWallisH) {
            const ranks = resultRanks(data, variables, dataGrouping, groupingVariable, group1, group2);
            result.ranks = JSON.stringify(ranks);
            console.log('Kruskal-Wallis Ranks:', JSON.stringify(ranks));

            const kruskalWallisH = resultKruskalWallisH(data, variables, dataGrouping, groupingVariable, group1, group2);
            result.kruskalWallisH = JSON.stringify(kruskalWallisH);
            console.log('Kruskal-Wallis H Test:', JSON.stringify(kruskalWallisH));
        }
