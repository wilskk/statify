...
function computeDescriptiveStats(variableData, displayStatistics, variables) {
    ...
    variableData.forEach((v, index) => {
        ...
        if (descriptive) {
            ...
            row["Mean"] = mean.toFixed(variables[index].decimals + 2);
            row["Std. Deviation"] = stdDev.toFixed(variables[index].decimals + 3);
            row["Minimum"] = Math.min(...data).toFixed(variables[index].decimals);
            row["Maximum"] = Math.max(...data).toFixed(variables[index].decimals);
        }

        // Tambahkan statistik quartiles jika opsi quartiles diaktifkan
        if (quartiles) {
            row["25th"] = calculatePercentile(data, 25).toFixed(variables[index].decimals + 2);
            row["50th"] = calculatePercentile(data, 50).toFixed(variables[index].decimals + 2);
            row["75th"] = calculatePercentile(data, 75).toFixed(variables[index].decimals + 2);
        }
