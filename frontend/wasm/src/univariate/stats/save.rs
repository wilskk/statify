...
pub fn save_variables(
    ...
    // Step 1: Create design matrix, response vector, and weights
    ...
    // Step 2: Create cross-product matrix and perform sweep
    ...
    // Calculate unweighted hat matrix diagonal elements: h_i = x_i' * (X'WX)^-1 * x_i
    ...
    for i in 0..n {
        ...
        let h_i = h_diag[i]; // x_i' * (X'WX)^-1 * x_i
        let leverage = weight * h_i;

        // Common term for studentized residuals, deleted residuals, and Cook's D
