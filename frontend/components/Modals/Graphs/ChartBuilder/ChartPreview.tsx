...
const ChartPreview = forwardRef<ChartPreviewRef, ChartPreviewProps>(
  (
    ...
    useEffect(() => {
      ...
      chartType,
      JSON.stringify(sideVariables),
      JSON.stringify(side2Variables),
      JSON.stringify(bottomVariables),
      JSON.stringify(bottom2Variables),
      JSON.stringify(colorVariables),
      JSON.stringify(filterVariables),
      JSON.stringify(lowVariables),
      JSON.stringify(highVariables),
      JSON.stringify(closeVariables),
      data,
      useaxis,
      width,
      height,
    ...
    const getGeneratedChartJSON = () => {
      return generatedChartJSON;
    };

    // Expose the function to parent component
    useEffect(() => {
      // You can add this to a ref or callback prop
    ...
    const createChartConfig = (
      ...
      const defaultData: Record<string, any[]> = {
        ...
        "Error Bar Chart": [
          { category: "Group A", subcategory: "Control", value: 25, error: 5 },
          {
            category: "Group A",
          ...
          { category: "Group B", subcategory: "Control", value: 20, error: 4 },
          {
            category: "Group B",
            subcategory: "Treatment",
            value: 35,
          ...
          { category: "Group C", subcategory: "Control", value: 28, error: 2 },
          {
            category: "Group C",
            subcategory: "Treatment",
            value: 32,
    ...
    // Helper function untuk stacked chart config
    const createStackedChartConfig = (
      chartType: string,
      isDefault: boolean = false
    ) => {
      const config = createChartConfig(chartType, isDefault);
    ...
    // Helper function untuk dual axis chart config
    const createDualAxisChartConfig = (
      chartType: string,
      isDefault: boolean = false
    ) => {
      const config = createChartConfig(chartType, isDefault);
    ...
    // Helper function untuk error bar chart config
