...
import { InfoIcon, ChevronDownIcon, ArrowLeft, HelpCircle, Loader2 } from "lucide-react";
...
import { useImportCsvProcessor } from "../hooks/useImportCsvProcessor"; // Updated import path
import { 
    DelimiterOption, 
    DecimalOption, 
    TextQualifierOption, 
    SelectOption 
} from "../types"; // Corrected import path from ../types to ./types

// Types (can be moved to a .types.ts file if they grow)
// export type DelimiterOption = CSVProcessingOptions['delimiter']; // Moved
// export type DecimalOption = CSVProcessingOptions['decimal']; // Moved
// export type TextQualifierOption = CSVProcessingOptions['textQualifier']; // Moved

// interface SelectOption { // Moved
//     value: string;
//     label: string;
// }
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="px-6 py-3 border-t border-border flex items-center justify-between bg-secondary flex-shrink-0">
                {/* Kiri: Help icon + text, text hilang di layar kecil */}
                <div className="flex items-center text-muted-foreground cursor-pointer hover:text-primary transition-colors">
                    <HelpCircle size={18} className="mr-1" />
                    {/* <span className="hidden sm:inline text-xs">Need help importing CSV?</span> */}
                </div>
                {/* Kanan: tombol Back/Import */}
                <div>
                    <Button variant="outline" onClick={onBack} disabled={hookIsProcessing} className="mr-2">
                        Back
                    </Button>
                    <Button variant="outline" onClick={handleReset} disabled={hookIsProcessing} className="mr-2">
                        Reset
                    </Button>
                    <Button
                        onClick={handleOk}
                        disabled={hookIsProcessing || !!submissionError}
                        {...(hookIsProcessing ? { loading: true } : {})}
                    >
