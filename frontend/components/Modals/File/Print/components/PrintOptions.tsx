...
import { Loader2, Printer, HelpCircle } from "lucide-react";
import { PrintOptionsProps, PaperSize, SelectedOptions } from "../types"; // sebelumnya "../types/types"
        ...
        <div className="flex flex-col h-full"> {/* Main wrapper for flex layout */}
            ...
            <div className={`p-6 flex-grow overflow-y-auto ${isMobile ? 'space-y-5' : 'space-y-4'}`}>
                ...
                <div className={`${isMobile ? 'flex flex-col space-y-1.5' : 'grid grid-cols-4 items-start gap-4'}`}>
                    <Label className={`${!isMobile ? 'text-right pt-1' : 'text-sm font-medium'}`}>
                        Content to Print
                    </Label>
            ...
            <div className="px-6 py-3 border-t border-border flex items-center justify-between bg-secondary">
                ...
                {/* Kanan: tombol Cancel/Print */}
                <div>
                    <Button variant="outline" onClick={onCancel} disabled={isGenerating} className="mr-2">
                        Cancel
                    </Button>
                    <Button variant="outline" onClick={onReset} disabled={isGenerating} className="mr-2">
                        Reset
                    </Button>
                    <Button
                        onClick={onPrint}
