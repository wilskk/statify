...
interface RangeSelection {
  ...
  advancedRangeFeatures: {
    ...
    conditionalRanges: {
      ...
      dynamicRangeAdjustment: (range: Range, data: DataRow[]) => AdjustedRange;
    };
  };
}
```
...
interface FilterVariableUsage {
  ...
  // Filter logic
  filterLogic: {
    applyFilter: (data: DataRow[], filterVariable: Variable) => FilteredData;
    ...
    combineFilters: (filters: FilterVariable[]) => CombinedFilter;
...
interface SelectionService {
  ...
  qualityAssurance: {
    ...
    generateSelectionSummary: (result: SelectionResult) => SelectionSummary;
...
- **Umpan Balik Visual**: Memberikan highlight visual secara _real-time_ pada baris data yang akan dipilih saat ekspresi sedang dibuat.
