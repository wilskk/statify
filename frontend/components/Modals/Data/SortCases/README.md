...
interface SortingService {
  ...
  qualityAssurance: {
    ...
    generateSortReport: (sortResult: SortResult) => SortOperationReport;
  };
}
```
...
const optimizeMemoryUsage = (dataSize: number, availableMemory: number) => {
...
- **`index.tsx` (Orchestrator)**:
  ...
  - Memanggil _hook_ `useSortCases` untuk mendapatkan state dan _handler_.
  - Merender `SortCasesUI` dan meneruskan _props_ yang diperlukan.
...
- **`useSortCases.ts` (Hook Logika)**:
  ...
  - Menyediakan _handler_ untuk memindahkan variabel, mengubah urutan, dan mengubah arah pengurutan.
  - Memanggil `sortData` dari `useDataStore` saat pengguna mengonfirmasi tindakan.

- **`SortCasesUI.tsx` (Komponen UI)**:
  - Komponen presentasi murni yang menerima semua data dan _handler_ dari `index.tsx`.
...
- **`types.ts` (Definisi Tipe)**:
  - Mendefinisikan _interface_ seperti `SortVariableConfig` (untuk menyimpan variabel dan arah pengurutannya).
  - Mendefinisikan _props_ untuk `SortCasesModal` dan `SortCasesUI`, memastikan keamanan tipe.
...
3.  **Penyimpanan**:
    ...
    - _Handler_ `handleOk` di dalam _hook_ dipanggil.
