...
describe('sortVarsService', () => {
  describe('sortDataColumns', () => {
    const originalVariables: Variable[] = [
      { id: 1, name: 'VarA', columnIndex: 0, type: 'NUMERIC', width: 8, decimals: 2, label: '', values: [], missing: null, columns: 8, align: 'left', measure: 'scale', role: 'input' },
      { id: 2, name: 'VarB', columnIndex: 1, type: 'STRING', width: 8, decimals: 0, label: '', values: [], missing: null, columns: 8, align: 'left', measure: 'nominal', role: 'input' },
      { id: 3, name: 'VarC', columnIndex: 2, type: 'NUMERIC', width: 8, decimals: 2, label: '', values: [], missing: null, columns: 8, align: 'left', measure: 'scale', role: 'input' },
    ];
    ...
    it('should correctly reorder data columns based on sorted variables', () => {
      ...
      const sortedVariables: Variable[] = [
        { ...originalVariables[2], columnIndex: 0, id: 3 }, // VarC -> index 0
        { ...originalVariables[1], columnIndex: 1, id: 2 }, // VarB -> index 1
        { ...originalVariables[0], columnIndex: 2, id: 1 }, // VarA -> index 2
      ];
    ...
    it('should handle cases where row length is shorter than expected', () => {
      ...
      const sortedVariables: Variable[] = [
        { ...originalVariables[1], columnIndex: 0, id: 2 }, // VarB -> index 0
        { ...originalVariables[0], columnIndex: 1, id: 1 }, // VarA -> index 1
        { ...originalVariables[2], columnIndex: 2, id: 3 }, // VarC -> index 2
      ];
