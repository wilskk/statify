...
export interface RecodeRule {
    id: string;
    oldValueType: 'value' | 'systemMissing' | 'systemOrUserMissing' | 'range' | 'rangeLowest' | 'rangeHighest' | 'else';
...
export const RecodeSameVariablesModal: React.FC<RecodeSameVariablesModalProps> = ({
    ...
    useEffect(() => {
        ...
        const initialAvailable = allVariablesFromStore
            .filter(v => v.name !== "" && v.tempId && !recodeVarTempIds.has(v.tempId))
    ...
    const moveToLeftPane = useCallback((variable: Variable, targetIndex?: number) => {
        if (!variable.tempId) return;
        setVariablesToRecode(prev => {
    ...
    const reorderVariables = useCallback((source: 'available' | 'recodeList', reorderedList: Variable[]) => {
        if (source === 'available') {
