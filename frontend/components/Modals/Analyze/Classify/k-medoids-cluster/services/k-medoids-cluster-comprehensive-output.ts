...
function calculateMedoidDistanceMatrix(
    ...
    const k = medoidIndices.length;
    const distances: number[][] = Array(k).fill(0).map(() => Array(k).fill(0));
    
    for (let i = 0; i < k; i++) {
        for (let j = i + 1; j < k; j++) {
            const medoid1 = medoidIndices[i];
            const medoid2 = medoidIndices[j];
            
            const point1 = variables.map(v => dataVariables[medoid1][v.columnIndex as number]);
            const point2 = variables.map(v => dataVariables[medoid2][v.columnIndex as number]);
...
export async function generateComprehensiveKMedoidsOutput(
    ...
    try {
        ...
        if (result.silhouette_scores && result.silhouette_scores.length === n) {
            ...
            silhouetteScores = await calculateSilhouetteScoresAsync(dataMatrix, safeLabels);
        }
        ...
        const assignments: ObjectAssignment[] = validRowIndices.map((origIdx, filtIdx) => {
            ...
            if (wasmDistances) {
                ...
                const medoidPoint = medoidFiltIdx != null && dataMatrix[medoidFiltIdx] ? dataMatrix[medoidFiltIdx] : [];
                const objectPoint = dataMatrix[filtIdx] || [];
        ...
        // Use WASM total cost as authoritative (matches R pam()$objective["swap"]).
        // Only fall back to JS sum when WASM cost is missing/zero.
        const wasmTotalCost: number = typeof result.cost === 'number' && result.cost > 0
            ? result.cost
            : 0;
        const jsTotalCost = assignments.reduce((sum, obj) => sum + (obj.distanceToMedoid || 0), 0);
        const calculatedTotalCost = wasmTotalCost > 0 ? wasmTotalCost : jsTotalCost;

        // cost_history layout: [0] = BUILD cost (initial, before any swap),
        // [1..n] = cost after each SWAP iteration. iteration_history mirrors this:
        // index 0 (iteration=0) = BUILD phase cost, last entry = final SWAP cost.
        const buildCost: number | undefined = result.iteration_history && result.iteration_history.length > 0
        ...
        const summary: KMedoidsSummary = {
            numClusters: k,
            totalCost: calculatedTotalCost,  // Use calculated cost for consistency
            buildCost,
            swapCost: calculatedTotalCost,
        ...
        const iterationHistory: IterationHistory[] = rawIterHistory
            ? rawIterHistory.map((item, idx) => {
                  ...
                  // For last iteration, use calculated total cost for consistency
                  const isLastIteration = idx === rawIterHistory.length - 1;
                  const finalCost = isLastIteration ? calculatedTotalCost : cost;
            ...
            : [{ iteration: 0, totalCost: calculatedTotalCost, improvement: 0, swapsMade: 0, medoids: result.medoids }];
        ...
        // Calculate medoid distance matrix
        // Pass original (unfiltered) indices so dataVariables[origIdx] is correct.
        ...
        allTables.push({
            ...
            rows: [
                ...
                { rowHeader: [], Metric: "Total Cost", Value: calculatedTotalCost.toFixed(4) },
        ...
        allTables.push({
            ...
            rows: medoids.map(medoid => ({
                ...
                ...Object.fromEntries(
                    variables.map(v => [v.name, typeof medoid.attributes[v.name] === 'number' 
                        ? (medoid.attributes[v.name] as number).toFixed(4)
                        : medoid.attributes[v.name]])
                )
            }))
        });
