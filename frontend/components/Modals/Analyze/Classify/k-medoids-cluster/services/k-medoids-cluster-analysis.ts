...
import { ClusterMode, AutoKMethod } from "@/components/Modals/Analyze/Classify/k-medoids-cluster/types/k-medoids-cluster";
...
// Import WASM module for fallback (direct execution)
...
async function initializeWasm() {
    if (!wasmInitialized) {
        try {
            ...
            console.log("Connection test:", test_connection());
        } catch (error) {
...
function mapWasmOutputToResult(wasmOutput: any): ClusteringResult {
    ...
    return {
        ...
        iterations: wasmOutput.iterations || 0,
...
export async function analyzeKMedoidsCluster({
    ...
    const n_init = configData.iterate.NumberOfInitializations || 10;
    
    console.log("K-Medoids Cluster Analysis Starting...");
    console.log("Config:", configData);
    console.log("Dataset:", { n: n, dimensions: d, k: k, n_init: n_init });
    ...
    try {
        ...
        // Mirror k-means style missing handling options.
        ...
        const preprocessingSummary: PreprocessingSummary = {
            ...
            afterPreprocessingN: dataMatrix.length,
        ...
        if (dataMatrix.length < 2) {
            throw new Error(`Not enough valid data points for clustering (found ${dataMatrix.length}). Check that the selected variables contain numeric values.`);
        }

        const finalMatrix = dataMatrix;
        ...
        console.log("🔍 Data Matrix Prepared:", {
            ...
            cols: dataMatrix[0]?.length,
            hasValidData: dataMatrix.length > 0 && dataMatrix[0].length > 0,
        });
        
        // Convert enum types to string literals for worker compatibility
        const method = (configData.iterate.Method || "PAM") as ClusteringMethod;
        const distanceMetric = (configData.main.DistanceMetric?.toLowerCase() || "euclidean") as DistanceMetric;
        ...
        if (isAutomatic) {
            ...
            if (autoWorker) {
                ...
                const wasmMod = await import("../rust/pkg/wasm");
                if (typeof (wasmMod as any).run_k_medoids_range !== "function") {
                    throw new Error("run_k_medoids_range not found in WASM build — please rebuild WASM with wasm-pack.");
                }
                const rawItems: any[] = (wasmMod as any).run_k_medoids_range(rangeInput);
            ...
            if (autoWorker) {
                ...
                finalResult = mapWasmOutputToResult(run_k_medoids(finalInput));
            }
            ...
            // Generate comprehensive output (await to ensure it's completed)
            ...
            const clusteringInput: ClusteringInput = {
                data: finalMatrix,
                n_clusters: configData.main.Cluster || 2,
                method: method,
                max_iterations: configData.iterate.MaximumIterations || 100,
                distance_metric: distanceMetric,
                random_seed: configData.iterate.RandomSeed || null,
                n_init: configData.iterate.NumberOfInitializations || 10,
                convergence_tolerance: configData.iterate.ConvergenceCriterion || 0.0,
            ...
            if (useWorker) {
                ...
                if (workerInstance) {
                    ...
                    const wasmResult = run_k_medoids(clusteringInput);
