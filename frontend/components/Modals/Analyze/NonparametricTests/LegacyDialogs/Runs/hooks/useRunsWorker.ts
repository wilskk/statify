...
import { RunsWorkerProps, RunsWorkerHookResult, WorkerInput, RunsWorkerResult, WorkerCalculationPromise } from '../types';

export const useRunsWorker = (props?: RunsWorkerProps): RunsWorkerHookResult => {
  const workerUrl = props?.workerUrl || '/workers/Runs/index.js';
  ...
  const calculate = useCallback((input: WorkerInput): Promise<RunsWorkerResult | null> => {
    setError(undefined);
    setIsCalculating(true);
    
    // Create a new promise to handle the worker response
    return new Promise<RunsWorkerResult | null>((resolve, reject) => {
