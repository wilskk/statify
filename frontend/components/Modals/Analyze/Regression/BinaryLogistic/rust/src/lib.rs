...
pub fn calculate_binary_logistic(
    ...
    let result_struct = match config.method {
        // Panggil fungsi run yang ada di strategies/enter.rs
        RegressionMethod::Enter => strategies::enter::run(&x_matrix, &y_vector, &config)
            .map_err(|e| api_error(&format!("Error di Metode Enter: {}", e)))?,

        // Placeholder untuk Stepwise nanti
        // RegressionMethod::ForwardConditional => strategies::stepwise::run(...)?
        _ => return Err(api_error("Metode belum diimplementasikan")),
    };
