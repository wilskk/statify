...
fn calculate_model_if_term_removed(
    ...
    for &g_idx in included_group_indices {
        ...
        // SPSS convention: when include_constant=false and removing this group leaves
        // zero parameters (no other groups remain), the model is completely empty.
        // SPSS reports LL = 0.0 for this degenerate case instead of n*ln(0.5).
        let other_groups_exist = included_group_indices.iter().any(|&gi| gi != g_idx);
        if !config.include_constant && !other_groups_exist {
            conditional_ll = 0.0;
        }
