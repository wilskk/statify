...
pub fn run(
    ...
    loop {
        ...
        // B. BACKWARD REMOVAL (Conditional - Wald Test)
...
fn calculate_step_snapshot(
    ...
    let remainder_test = if !variables_not_in.is_empty() {
        if included_indices.is_empty() {
            // Jika Step 0 (Null Model), gunakan Global Score Test (Matriks)
            ...
            // Jika Step > 0 (model sudah ada variabel), gunakan penjumlahan score (Standard approximation)
            // Karena menghitung Residual Chi-Square matriks untuk subset variabel jauh lebih kompleks
            // dan aproksimasi ini standar di banyak software.
