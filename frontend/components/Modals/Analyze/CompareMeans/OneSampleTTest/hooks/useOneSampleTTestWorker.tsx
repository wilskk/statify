...
import { OneSampleTTestWorkerProps, OneSampleTTestWorkerHookResult, WorkerInput, OneSampleTTestWorkerResult, WorkerCalculationPromise } from '../types';

export const useOneSampleTTestWorker = (props?: OneSampleTTestWorkerProps): OneSampleTTestWorkerHookResult => {
  const workerUrl = props?.workerUrl || '/workers/OneSampleTTest/index.js';
  ...
  const calculate = useCallback((input: WorkerInput): Promise<OneSampleTTestWorkerResult | null> => {
    setError(undefined);
    setIsCalculating(true);
    
    // Create a new promise to handle the worker response
    return new Promise<OneSampleTTestWorkerResult | null>((resolve, reject) => {
