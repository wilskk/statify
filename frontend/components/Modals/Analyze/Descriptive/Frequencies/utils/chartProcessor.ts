...
export const processAndAddCharts = async (
    ...
    for (const varName in frequencyTables) {
        ...
        // === New: build standardized ChartJSON with ChartService (bar & pie) ===
        let outputData: any;
        try {
            if (chartOptions.type && chartOptions.type !== "histograms") {
                const chartType = chartTypeMap[chartOptions.type];
                // Build chart data expected by ChartService → [{ category, value }]
                const processedChartData = table.rows.map(row => ({
                    category: row.label,
                    value: chartOptions.values === 'percentages' ?
                        (row.validPercent ?? row.percent ?? 0) : row.frequency,
                }));
                ...
                const chartJSON = ChartService.createChartJSON({
                    ...
                    chartConfig: {
                        useAxis: chartType !== "Pie Chart",
                    },
                });
                outputData = chartJSON;
            } else {
                // ❗ Legacy fallback for histogram or undefined type
                outputData = {
                    data: chartData,
                    options: {
            ...
            // Fallback to legacy structure if ChartService fails
            outputData = {
                data: chartData,
                options: {},
            };
